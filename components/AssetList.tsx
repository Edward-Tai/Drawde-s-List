...
const AssetList: React.FC<AssetListProps> = ({ assets, onDeleteMultiple, onMove, onUpdate, onUpdateMultiple }) => {
              ...
              {groupAssets.length === 0 ? (
                ...
                groupAssets.map((asset) => {
                  ...
                  return (
                    ...
                    >
                      {isSelectionMode && <td className="p-3 align-top text-center border-r border-transparent" onClick={(e) => e.stopPropagation()}><div className="mt-1 flex justify-center"><input type="checkbox" checked={isSelected} onChange={() => toggleSelect(asset.id)} className="w-4 h-4 rounded border-studio-600 bg-studio-900 text-studio-accent cursor-pointer accent-studio-accent" /></div></td>}
                      <td className="p-3 align-top overflow-visible border-r border-transparent" onClick={e => (isStyleMode || isEditingAny) && e.stopPropagation()}>
                        <div className="mb-1 relative" onClick={(e) => { if (isSelectionMode || isStyleMode || !asset.image || editingCell?.id === asset.id) return; e.stopPropagation(); const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setPreviewData({ id: asset.id, url: asset.image!, top: rect.bottom + 5, left: rect.left + 10 }); }} onMouseLeave={() => setPreviewData(null)} onDoubleClick={(e) => { if (isSelectionMode || isStyleMode) return; e.stopPropagation(); setEditingCell({ id: asset.id, field: 'name' }); setPreviewData(null); }} title={isStyleMode ? "" : (asset.image ? "點擊顯示圖片預覽 / 雙擊編輯名稱" : "雙擊編輯名稱")}>
  ...
  return (
    <div className="space-y-6 flex flex-col h-full" onClick={(e) => {
      ...
      {previewData && createPortal(<div className="fixed z-[9999] p-1 bg-studio-800 border border-studio-600 rounded-lg shadow-2xl animate-in fade-in zoom-in duration-200 pointer-events-none" style={{ top: previewData.top, left: previewData.left, maxWidth: '300px', maxHeight: '300px' }}><img src={previewData.url} alt="Preview" className="w-full h-full object-contain rounded" /></div>, document.body)}
    </div>
  );
};
